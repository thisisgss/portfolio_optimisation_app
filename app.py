...
if 'risky_asset_percent' not in st.session_state:
    st.session_state.risky_asset_percent = 70 # Default to 70% risky
...
def calculate_cal_metrics(max_sharpe_portfolio, risk_free_rate, y_star):
    """Calculates the return and risk for the personalized portfolio (P) on the CAL."""
    Rp = max_sharpe_portfolio['Expected Return'] * y_star + risk_free_rate * (1 - y_star)
    StdDev_p = max_sharpe_portfolio['Risk (Std Dev)'] * y_star
    return Rp, StdDev_p

# --- UI HELPER FUNCTION ---
def render_home_button():
    """Renders a floating Home button."""
    # Only render if not on the main page
    if st.session_state.page != 'main':
        if st.button("🏠 Home", key="home_btn_global"):
            st.session_state.page = 'main'
            st.rerun()

# --- UI RENDERING FUNCTIONS ---
def render_main_page():
    render_home_button() # Keep this inside the function for the side bar config
        ...
        with tab1:
            if data['sim_results_df'] is not None:
                ...
                fig.add_trace(go.Scatter(
                    x=data['sim_results_df']['Risk (Std Dev)'], 
                    y=data['sim_results_df']['Expected Return'], 
                    mode='markers', 
                    marker=dict(
                        size=5, 
                        opacity=0.3, 
                        color=data['sim_results_df']['Sharpe Ratio'], 
                        colorscale=px.colors.sequential.Viridis, 
                        showscale=True
                    ),
                    name='All Portfolios (Sharpe Ratio)'
                ))
...
def render_questionnaire():
    # This function is now just a pass-through to the personalized results page
    st.session_state.page = 'personalized_results'
    st.rerun()
...
def render_personalized_results():
    ...
    st.header("✨ Personalize Your Portfolio Allocation")
    st.markdown("Use the slider below to select the percentage of your capital to allocate to the **Optimized Risky Assets** (your selected stocks).")

    # Slider for Risky Asset Percentage
    risky_asset_percent = st.slider(
        "**Optimized Risky Assets Allocation** (%)",
        min_value=0,
        max_value=100,
        value=st.session_state.risky_asset_percent,
        step=5,
        format="%d%%",
        key='risky_asset_slider',
        help="The percentage of your total portfolio you wish to allocate to the optimized mix of stocks."
    )
    
    # Save the current slider value
    st.session_state.risky_asset_percent = risky_asset_percent
    
    # Calculate weights based on slider
    y_star = risky_asset_percent / 100
    risk_free_percent = 100 - risky_asset_percent
    
    # Determine the risk profile label based on the slider value
    if risky_asset_percent <= 30:
        profile = "Conservative"
        emoji = "🛡️"
    elif risky_asset_percent <= 80:
    ...
    st.markdown(f"**Current Profile:** **{profile}** {emoji}")
    st.markdown(f"This translates to **{risk_free_percent:.0f}%** invested in a **Risk-Free Asset** (e.g., Cash, Treasury Bills).")
    st.markdown("---")


    # --- Personalized Portfolio Calculation ---
    
    risky_weights_normalized = data['max_sharpe_portfolio'][data['valid_symbols']]
    personalized_weights = risky_weights_normalized * y_star
    personalized_weights['Risk-Free Asset'] = 1 - y_star
    st.session_state.portfolio_data['personalized_weights'] = personalized_weights

    # Calculate Personalized Portfolio Metrics
    Rp_p, StdDev_p = calculate_cal_metrics(data['max_sharpe_portfolio'], data['risk_free_rate'], y_star)

    # --- Capital Allocation Line (CAL) Chart ---
    st.subheader("Capital Allocation Line (CAL)")
    
    max_sharpe_portfolio = data['max_sharpe_portfolio']
    risk_free_rate = data['risk_free_rate']
    
    # Risky Portfolio (Max Sharpe) M
    StdDev_m = max_sharpe_portfolio['Risk (Std Dev)']
    Rp_m = max_sharpe_portfolio['Expected Return']
    
    # Risk-Free Asset F
    StdDev_f = 0
    Rp_f = risk_free_rate

    # Generate CAL points (from 0% risky up to 150% risky for visualization)
    y_values = np.linspace(0, 1.5, 10)
    cal_std_dev = [StdDev_m * y for y in y_values]
    cal_returns = [Rp_m * y + Rp_f * (1 - y) for y in y_values]
    
    cal_df = pd.DataFrame({'Risk (Std Dev)': cal_std_dev, 'Expected Return': cal_returns})

    # Plotting
    fig_cal = go.Figure()

    # 1. Efficient Frontier (if available) 
    if data['sim_results_df'] is not None:
        fig_cal.add_trace(go.Scatter(
            ...
            name='Efficient Frontier Simulations'
        ))

    # 2. Capital Allocation Line (CAL)
    fig_cal.add_trace(go.Scatter(
        x=cal_df['Risk (Std Dev)'], 
        y=cal_df['Expected Return'], 
        mode='lines', 
        line=dict(color='yellow', width=3, dash='dash'),
        name='Capital Allocation Line (CAL)'
    ))
    
    # 3. Personalized Portfolio P - FIX: Removed percentage from legend name
    fig_cal.add_trace(go.Scatter(
        x=[StdDev_p], 
        y=[Rp_p], 
        mode='markers', 
        marker=dict(color='#82C0E6', size=15, symbol='circle', line=dict(width=2, color='DarkSlateGrey')), 
        name='Your Portfolio (P)' # Fixed: Removed percentage from legend
    ))

    # 4. Max Sharpe Portfolio M
    fig_cal.add_trace(go.Scatter(
        x=[StdDev_m], 
        y=[Rp_m], 
        mode='markers', 
        marker=dict(color='red', size=15, symbol='star'), 
        name='Max Sharpe Portfolio (M)'
    ))
    ...
    fig_cal.add_trace(go.Scatter(
        ...
        y=[Rp_f], 
        mode='markers', 
        marker=dict(color='lightgreen', size=15, symbol='square'), 
        name='Risk-Free Asset (F)'
    ))

    fig_cal.update_layout(
        title='Capital Allocation Line: Risk-Return Trade-off',
        xaxis_title='Portfolio Risk (Standard Deviation)',
        yaxis_title='Expected Portfolio Return',
        height=600,
        xaxis_tickformat=".2%", 
    ...
    with col1:
        # Filter out assets with near-zero weight for cleaner visualization
        weights_for_pie = personalized_weights[personalized_weights > 0.001]
        fig_pie = px.pie(
            values=weights_for_pie.values,
            names=[data['ticker_to_name'].get(i, i) for i in weights_for_pie.index],
            title='Personalized Asset Allocation Mix',
            hole=.3
        )
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("⬅️ Back to Main Results", use_container_width=True):
            st.session_state.page = 'main'
            st.rerun()
    with c2:
        # FIX: Added unique key for reliable navigation
        if st.button("💰 Proceed to Investment Plan", type="primary", use_container_width=True, key='go_to_investment_1'):
...
def render_investment_plan():
    ...
    if investment_amount and 'personalized_weights' in data:
        with st.spinner("Fetching latest prices for allocation..."):
            try:
                latest_prices_data = yf.download(data['valid_symbols'], period='1d', progress=False)['Close']
                if isinstance(latest_prices_data, pd.Series):
                    latest_prices = latest_prices_data.iloc[-1]
                else:
                    latest_prices = latest_prices_data.iloc[-1]
                ...
                latest_prices_series = pd.Series(latest_prices, index=data['valid_symbols'])
                allocation_df['Latest Price'] = latest_prices_series.reindex(allocation_df.index)
                allocation_df.loc['Risk-Free Asset', 'Latest Price'] = 1.0 # Placeholder for cash value
                ...
                invested_amount_stocks = (allocation_df.drop('Risk-Free Asset', errors='ignore')['Number of Shares'] * allocation_df.drop('Risk-Free Asset', errors='ignore')['Latest Price']).sum()
                initial_risk_free_amount = allocation_df.loc['Risk-Free Asset', 'Amount to Invest'] if 'Risk-Free Asset' in allocation_df.index else 0
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("⬅️ Back to Personalization"):
            st.session_state.page = 'personalized_results'
            st.rerun()
    with c2:
        # FIX: Added unique key for reliable navigation
        if st.button("📈 Proceed to Backtesting", type="primary", use_container_width=True, key='go_to_backtesting_1'):
...
def render_backtesting_page():
    ...
    with st.spinner("Running backtest..."):
        ...
        risky_weights_normalized = risky_weights / y_star if y_star > 0 else pd.Series(0, index=risky_weights.index)
        ...
        df_plot = pd.DataFrame({'Your Personalized Portfolio': portfolio_cumulative_returns}).dropna()
        ...
        if benchmark_ticker:
            try:
                ...
                if not benchmark_data.empty:
                    ...
                    df_plot['Benchmark'] = benchmark_cumulative_returns
                else:
                    st.warning(f"Could not find data for benchmark: {benchmark_ticker}")
            except Exception as e:
                st.warning(f"Error fetching benchmark data: {e}")
